          ...
          <div className="space-y-32">
             ...
             <div className="space-y-40">
                {categoriesWithProducts
                  ...
                  .map((item, idx) => {
                    const isPlaceholder = !item.image || item.image === '/placeholder.jpg' || item.image === '/placeholder.svg'
                    const isLandscapeLayout = 
                      item.name.toUpperCase().includes('MANIFOLD') || 
                      item.id === 'mt-102' || 
                      item.id === 'ga-104' || 
                    ...
                    if (isLandscapeLayout) {
                       return (
                         <div key={item.id} id={item.id} className="flex flex-col gap-12 group/item scroll-mt-40">
                            ...
                            <div className="w-full aspect-[16/9] lg:aspect-[21/9] bg-[#F8F9FA] rounded-[3rem] relative flex items-center justify-center p-12 overflow-hidden group-hover/item:shadow-[0_40px_100px_rgba(0,0,0,0.08)] transition-all duration-700">
                               <Image 
                                 src={item.image} 
                                 alt={item.name}
                                 fill
